...
var paletteColors map[string]string = map[string]string{
	"white":  "#e4e4e4",
	"black":  "#121212",
	"gray0":  "#262626",
	"gray1":  "#444444",
...
var ansiColors map[int]string = map[int]string{
	0:   "#000000",
	1:   "#800000",
	2:   "#008000",
	3:   "#808000",
...
func FindAnsiColor(c colors.Color) (int, colors.Color) {
	result := colors.Color{}
	id := -1
	distance := 99999999999999.

	for index, color_string := range ansiColors {
		localDistance := c.ColorDistance(color_string)

		if localDistance < distance {
			distance = localDistance
			result = colors.MakeColor(color_string)
			id = index
		}
	}
...
func PrintVimColors() {
	for _, color_key := range paletteKeys {
		c := colors.MakeColor(paletteColors[color_key])
		a_id, _ := FindAnsiColor(c)
		fmt.Printf("\" Ge%s\n", strings.Title(color_key))
		fmt.Printf("hi Ge%s guifg=%s guibg=NONE guisp=NONE gui=NONE ctermfg=%d ctermbg=NONE cterm=NONE\n",
			strings.Title(color_key), c.ToHex(), a_id)
		fmt.Printf("hi Ge%sBold guifg=%s guibg=NONE guisp=NONE gui=bold ctermfg=%d ctermbg=NONE cterm=bold\n",
			strings.Title(color_key), c.ToHex(), a_id)
...
func PrintPalette() {
	fmt.Println("RGB 24Bit -> ANSI")
	for _, color_key := range paletteKeys {
		c := colors.MakeColor(paletteColors[color_key])
		a_id, a := FindAnsiColor(c)
		fmt.Printf(
			"%s - %s%s%s%s -> %s%s%s%s (%d)\n",
			color_key,
			c.FindForeground(), c.TermBg(), c.ToHex(), c.Reset(),
			a.FindForeground(), a.TermBg(), a.ToHex(), a.Reset(),
			a_id,
		)
	}
}
