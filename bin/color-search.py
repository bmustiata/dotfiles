...
def rgb2short(rgb):
    ...
    for i in range(256):
        dist = compute_color_distance(rgb, CLUT[i][1])
        if dist < color_distance:
            color_distance = dist
            color_index = i

    return CLUT[color_index]


def compute_color_distance(color1, color2):
    r1, g1, b1 = break_color(color1)
    r2, g2, b2 = break_color(color2)
...
def break_color(color):
    if color.startswith('#'):
        color = color[1:]

    return [ float(int(h, 16)) for h in re.split(r'(..)(..)(..)', color)[1:4] ]
